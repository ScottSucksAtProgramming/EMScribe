import os
import pytest
import logging
from commands.extract_command import ExtractCommand
from modules.transcript_extractor import TranscriptExtractor
from utils.utils import ensure_file_exists

class MockTranscriptExtractor:
    def extract(self, transcript):
        return "extracted data"

def test_extract_command_file_not_found(tmp_path, caplog):
    extractor = MockTranscriptExtractor()
    command = ExtractCommand(extractor)

    transcript_path = tmp_path / "nonexistent_file.txt"
    output_path = tmp_path / "output.txt"

    with caplog.at_level(logging.ERROR):
        command.execute(transcript_path, output_path)

    assert "File not found" in caplog.text

def test_extract_command_success(tmp_path):
    extractor = MockTranscriptExtractor()
    command = ExtractCommand(extractor)

    transcript_path = tmp_path / "transcript.txt"
    output_path = tmp_path / "output.txt"

    with open(transcript_path, "w") as file:
        file.write("raw transcript")

    command.execute(transcript_path, output_path)

    with open(output_path, "r") as file:
        extracted_data = file.read()
    
    assert extracted_data == "extracted data"
